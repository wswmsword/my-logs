...
如果设计得当，使用快捷键来聚焦一个元素或激活一个微件，可以显著提升页面常用功能的可用性。这个章节讨论对效率影响最大的快捷键设计和实现因素，包括了：
1. 理解快捷键是如何增强键盘界面的，理解是否让某个快捷键移动焦点、执行操作，或两者兼具；
2. 避免与辅助技术、浏览器和操作系统的按键绑定冲突；
3. 公开和编写键盘绑定文档。

### 设计快捷键的范围和行为

这个章节解释了在为元素和功能分配快捷键时以及各个快捷键的行为导致的因素：
1. 确保可以通过导航访问，快捷键增强访问，而不是取代标准的键盘访问；
2. 有效地从以下行为中选择：
	1. 导航：移动元素焦点。
	2. 激活：执行某个元素的操作，这个元素可能无焦点或不可见。
	3. 导航和激活：同时聚焦和激活某个元素。
3. 平衡效率和认知负担：缺少快捷键可能会降低效率，然而大量的快捷键又会提高认知负担，破坏体验。

#### 导航能确保基本的访问

在分配快捷键前，必须确保将被分配快捷键的特性和功能可以通过键盘正常访问。换句话讲，所有可能成为快捷键目标的元素，都要是可聚焦的，具体见这两个章节：
- [组件之间的键盘导航（Tab 序列）](#组件之间的键盘导航（Tab-序列）)
- [组件内部的键盘访问](#组件内部的键盘访问)

不用快捷键代替导航，这对全键盘访问是很重要的：
1. 使功能和快捷键被发现的主要方法是让目标元素可聚焦以及 revealing key assignments on the element itself。
2. 如果依赖键盘的人必须阅读文档来学习访问界面需要什么按键，那么界面也许在技术上符合可访问性标准，但实际上只有少部分人是可访问的，他们知道有这样的文档，他们有额外的时间阅读文档，他们有能力提取重要信息；
3. 不是所有以来键盘界面的设备支持快捷键。

#### 选择合适的快捷键行为

下面的约定可能有助于找到快捷键的最有利行为：
- 当主要目的是让移动焦点导航更有效率时，比如，减少用户按下 <kbd>Tab</kbd> 或方向键的次数。在给文本框、工具栏或复合组件（列表框、树形、表格、菜单栏）分配快捷键时也会期望这种行为。这种行为对于移动焦点到页面上的某个部分也很有用，比如移动到主体或补充信息；
- 在一个上下文中存在焦点时激活一个元素而不用聚焦。这种行为常见于指令按钮和一个不可见元素上的功能，例如通过菜单才能访问的“保存”选项。举个例子，如果焦点在列表框的某个选项上，工具栏上包含移除和移动选项的按钮，这是把焦点保留在列表框，通过快捷键执行工具栏的某个按钮是最有利的。这个行为对屏幕阅读器用户很重要，因为它提供了对所执行动作的确认，也让执行多个命令更有效率。举个例子，当屏幕阅读器用户按下“向上”的快捷键，用户会听到新选项，因为焦点仍然在列表中。类似的，当用户按下删除的快捷键，用户会听到下一条选项，并且立刻决定是否再次进行删除操作；
- 当快捷键目标仅有单个功能，并且这个功能也能被那个目标触发，则移动焦点并激活。这个行为通常在为打开菜单或对话框的按钮、在复选框、在导航链接和按钮上出现。

#### 选择添加快捷键的位置

本章节的草稿内容请跟踪 [issue 219](https://github.com/w3c/aria-practices/issues/219)。

设计键盘界面的首要目标就是在使用键盘基本导航时保持简单、高效以及符合直觉。如果键盘界面的基操低效，想要通过快捷键来弥补基础设计问题，例如布局或命令结构问题，然而快捷键不会减少用户的挫败感受。这一点的实际意义在于，在一个设计良好的用户界面里，对于那些要通过快捷键访问来创造最佳可用性的功能，占比不是很高。在许多简洁的用户界面里，快捷键完全是多余的。而且，若是界面的快捷键太多，多余的快捷键会造成认知负担，也会让最有用的快捷键难以记住。
...
在选择按键分配快捷键的时候，要考虑许多因素：
- 通过使用记忆法（例如，保存使用 <kbd>Control + S</kbd>）、遵循逻辑或空间模型，让快捷键易于学习和记忆；
- 对界面实行本地化，包括按键可用性的差异，按键行为的差异，包括会影响记忆的语言考虑因素；
- 避免和管理与辅助技术、浏览器和操作系统冲突的按键绑定。

设计支持利于学习和记忆的快捷键方案的方法已经超出了本指南范畴。除非快捷键使用广泛，否则模仿常见桌面端软件（例如浏览器）的行为就足够了。同样，本地化虽然很重要，但如何解决还是要留给专职研究此专题的资源。

本章节剩余部分提供平衡和按键冲突有关的需求和担忧的指南。通常最理想的按键分配是不和操作系统、浏览器及辅助技术发生冲突。冲突会阻碍用户高效访问某个必要功能，会让用户陷入完美的冲突风暴。同时，某些情况里故意冲突又是有用的。并且，有大量的操作系统、浏览器和辅助技术按键，没有冲突几乎不可能。所以无论有意无意，减少冲突都有必要。

> **Note**
> 以下章节中，<kbd>meta</kbd> 指 Windows 里的 <kbd>Windows</kbd>，指 macOS 下的 <kbd>Command</kbd>。

#### 操作系统按键冲突

避免和操作系统级别的按键发生冲突，比如应用和窗口管理，显式和声音控制。一般，这可以通过避免下面的按键绑定来达到预防冲突。

1. 任何修饰键 + <kbd>Tab</kbd>、<kbd>Enter</kbd>、<kbd>Space</kbd> 或 <kbd>Escape</kbd>
2. <kbd>Meta</kbd> + 任何其它按键
3. <kbd>Alt</kbd> + 功能键

此外，还有些重要的应用级别的功能，大部分应用都支持，例如浏览器。这些功能包括：
...
虽然辅助技术集成了上千个按键绑定，但避免冲突不难。这是因为辅助技术必须开发按键绑定方案，以此避免和操作系统与其他应用冲突。他们通过劫持特定的按键作为修饰键来定义按键命令。例如，许多辅助技术利用 <kbd>Caps Lock</kbd> 作为修饰键。

您可以绕开下面这些按键绑定来避免和辅助技术快捷键冲突。

1. <kbd>Caps Lock</kbd> + 其它按键组合
2. <kbd>Insert</kbd> + 其它按键组合
3. <kbd>Scroll Lock</kbd> + 其它按键组合
4. 仅 macOS，<kbd>Control+Option</kbd> + 其它按键组合

#### 浏览器按键冲突

虽然浏览器键盘方案之间有很大相似性，但方案内的模版却不太一样。因此要避免浏览器按键冲突要更加困难。虽然冲突的影响有时会功能的两种访问途径而有所减轻——键盘导航访问和快捷键，但和重度使用的功能的快捷键避免冲突还是很重要。特别注意以下快捷键冲突：
...
通常理想情况是不发生按键冲突，然而有的情况里刻意冲突却是可接受的，甚至是期望的。例如下面的几种情况：
